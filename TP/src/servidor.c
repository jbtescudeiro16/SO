    ...
    if (!(pedido = fork())) {
        ...
        if (cabeca->n_request == 4) {
            if (!(ultimo = fork())) {
                ...
                dup2(fd1, 0);
                int fd2 =
                    open(cabeca->request[1], O_WRONLY | O_CREAT | O_TRUNC);
                dup2(fd2, 1);
                char transform[100];
                snprintf(transform, sizeof(transform), "%s/%s", directory,
                         cabeca->request[2]);
                execl(transform, transform, NULL);
            }
        } else {
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < 7; ++i){
      printf("%s -> %d\n", configuracao.idTransf[i], configuracao.limit[i]);
    }
        ...
        if(n_bytes > 0){
            ...
            if(!strcmp("terminou", args[0])){
                ...
                for(int i = 0; i < last_process; ++i){
                  if(pedidos[i] == id){
                    for(int j = 0; j < 7; j++){
                        configuracao.current[j] -=
                            pedidos_struct[i]->current[j];
                    }
                  }
                }
            }
